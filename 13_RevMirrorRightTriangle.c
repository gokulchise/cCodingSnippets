/*!< C programme to print reverse mirrored right triangle */
...
#define IS_MIRROR_REV_RTRIANGLE(_r,_c,_n)		(_c <= (_r-1))
...
int main(void) {
	...
	for (row = 1; row <= limit; row ++) {
		...
		for (col = 1; col <= limit; col ++) {
			...
			IS_MIRROR_REV_RTRIANGLE(row, col, limit) ? PRINT_SPACE() : PRINT_STAR();
		}
